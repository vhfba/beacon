...
public class TriggerProbeActionUseCase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!isAssigned)
            throw new DomainException($"Plugin {pluginId} is not assigned to probe {probe.Id.Value}");

        var execution = new ProbeActionExecution(probe.Id, pluginId, input.TriggeredBy);
