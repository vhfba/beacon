...
static void get_bssid(char *mac_addr, unsigned char *arg) {
    sprintf(mac_addr, "%02x:%02x:%02x:%02x:%02x:%02x",
            arg[0], arg[1], arg[2], arg[3], arg[4], arg[5]);
}

static void parse_security(char *out, unsigned char *ie, int ielen) {
    int has_wpa = 0;
    int has_wpa2 = 0;
    int has_wpa3 = 0;
    int has_wep = 0;
    ...
    while (ielen >= 2 && ielen >= ie[1] + 2) {
        ...
        case 0x30:
            ...
            for (int i = 0; i < ie[1]; i++) {
                if (ie[i] == 0x8c) has_wpa3 = 1;
            }
            break;
        case 0xdd:
            if (ie[1] > 4 && ie[2] == 0x00 && ie[3] == 0x50 && ie[4] == 0xF2) {
                has_wpa = 1;
            }
            break;
        case 0x01:
    ...
    if (has_wpa3) strcpy(out, "WPA3");
    else if (has_wpa2 && has_wpa) strcpy(out, "WPA/WPA2");
    else if (has_wpa2) strcpy(out, "WPA2");
    else if (has_wpa) strcpy(out, "WPA");
    else if (has_wep) strcpy(out, "WEP");
...
static void get_ssid(char *out, unsigned char *ie, int ielen) {
    while (ielen >= 2 && ielen >= ie[1] + 2) {
        if (ie[0] == 0 && ie[1] <= 32) {
            int len = ie[1];
...
static int callback_dump(struct nl_msg *msg, void *arg) {
    ...
    if (bss[NL80211_BSS_FREQUENCY]) {
        ...
        dataResult[result_count].channel = freq_to_channel(dataResult[result_count].frequency);
    } else {
    ...
    parse_security(dataResult[result_count].security, nla_data(bss[NL80211_BSS_INFORMATION_ELEMENTS]), nla_len(bss[NL80211_BSS_INFORMATION_ELEMENTS]));

    if (bss[NL80211_BSS_SIGNAL_MBM]) dataResult[result_count].mbm = (int)nla_get_u32(bss[NL80211_BSS_SIGNAL_MBM]);
...
static void metrics(void) {
    ...
    for (int i = 0; i < result_count; i++) {
        ...
        if (i > 0) printf(",");
        printf(
            "{\"name\":\"beacon_wifi_scan_signal_dbm\",\"kind\":\"gauge\",\"value\":%.2f,\"labels\":{\"ssid\":\"%s\",\"bssid\":\"%s\",\"frequency\":\"%d\"}},"
            "{\"name\":\"beacon_wifi_scan_frequency_mhz\",\"kind\":\"gauge\",\"value\":%d,\"labels\":{\"ssid\":\"%s\",\"bssid\":\"%s\"}},"
            "{\"name\":\"beacon_wifi_scan_channel\",\"kind\":\"gauge\",\"value\":%d,\"labels\":{\"ssid\":\"%s\",\"bssid\":\"%s\"}},"
            "{\"name\":\"beacon_wifi_scan_connected\",\"kind\":\"gauge\",\"value\":%d,\"labels\":{\"ssid\":\"%s\",\"bssid\":\"%s\"}},"
            "{\"name\":\"beacon_wifi_scan_security\",\"kind\":\"gauge\",\"value\":1,\"labels\":{\"ssid\":\"%s\",\"bssid\":\"%s\",\"type\":\"%s\"}}",
            signal_dbm, ssid, bssid, dataResult[i].frequency,
            dataResult[i].frequency, ssid, bssid,
            dataResult[i].channel, ssid, bssid,
            dataResult[i].connected, ssid, bssid,
...
int main(int argc, char **argv) {
    ...
    if (argc > 1 && argv[1] && argv[1][0] != '\0') iface = argv[1];
